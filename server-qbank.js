...
module.exports = function (app, io, uploadsDir) {
  ...
  function computeLeaderboard(perfData) {
    return perfData
      .map((p) => {
        ...
        let examScore = 0;
        if (p.mark) {
          Object.values(p.mark).forEach((v) => {
            examScore += parseFloat(v) || 0;
          });
        }
        return {
          name: p.name,
  ...
  app.get("/api/mcq", (req, res) => {
    ...
    const { paper, chapter, boardOnly, importantOnly, amount, topics } =
      req.query;

    if (paper && paper !== "All") data = data.filter((d) => d.paper === paper);
    if (chapter && chapter !== "All")
      data = data.filter((d) => String(d.chapter) === chapter);
    if (boardOnly === "true")
      data = data.filter(
        (d) => String(d.board_question).toUpperCase() === "TRUE",
      );
    if (importantOnly === "true")
      data = data.filter(
        (d) => String(d.marked_important).toUpperCase() === "TRUE",
      );
    if (topics && topics !== "All")
      data = data.filter(
        (d) =>
          d.topics &&
          String(d.topics).toLowerCase().includes(topics.toLowerCase()),
      );
  ...
  app.get("/api/mcq/chapters", (req, res) => {
    ...
    data.forEach((d) => {
      if (d.chapter) chapters.add(String(d.chapter));
    });
  ...
  app.post("/api/mcq", (req, res) => {
    ...
    if (success) res.json({ status: "ok", record: newRecord });
      ...
      res
        ...
        .json({
          error:
            "Failed to write to Excel. Ensure the file is not open in another program.",
        });
  });
  ...
  app.delete("/api/mcq/:id", (req, res) => {
    ...
    if (record) {
      ...
      ["question_image", "passage_image", "question2_image"].forEach(
        (field) => {
          if (record[field]) {
  ...
  app.post("/api/mcq/bulk-upload", excelUpload.single("file"), (req, res) => {
    ...
    newRows = newRows.map((r) => ({
      ...r,
      id: r.id || `mcq_${Date.now()}_${Math.floor(Math.random() * 10000)}`,
    }));
  ...
  app.post("/api/mcq/export-to-game", (req, res) => {
    ...
    if (chapter && chapter !== "All")
      data = data.filter((d) => String(d.chapter) === chapter);
    if (boardOnly === true || boardOnly === "true")
      data = data.filter(
        (d) => String(d.board_question).toUpperCase() === "TRUE",
      );
    ...
    if (topics && topics !== "All")
      data = data.filter(
        (d) =>
          d.topics &&
          String(d.topics).toLowerCase().includes(String(topics).toLowerCase()),
      );
    ...
    if (data.length === 0) {
      return res
        .status(404)
        .json({ error: "No Type 1 MCQs matched the filter criteria." });
    }
  ...
  app.post(
    ...
    (req, res) => {
      ...
      newRows = newRows.map((r) => ({
        ...r,
        id: r.id || `cq_${Date.now()}_${Math.floor(Math.random() * 10000)}`,
      }));
      // REPLACE existing xlsx entirely
  ...
  async function generateQuestionPaperDocx(res, questions, type, meta) {
    try {
      ...
      const T = (text, opts = {}) =>
        new TextRun({ text: String(text || ""), ...opts });
      ...
      if (type === "cq") {
        ...
        questions.forEach((q) => {
          const blockRuns = [
            T(`প্রশ্ন ${bnNum(qNum)}।  `, { bold: true, size: 22 }),
          ];
          if (q.passage) blockRuns.push(T(q.passage, { size: 22 }));
          headerChildren.push(
            P(blockRuns, { spacing: { before: 80, after: 100 } }),
          );
          ...
          [
            { label: "ক", text: q.knowledge },
            { label: "খ", text: q.understanding },
            { label: "গ", text: q.application },
            { label: "ঘ", text: q.higher_thinking },
          ].forEach((sub) => {
            if (sub.text) {
              headerChildren.push(
                P(
        ...
        function renderMcqParas(qObj) {
          ...
          if ((qtype === 3 || qtype === 4) && (q.passage || q.passage_image)) {
            const hint =
              qtype === 4 && q.question2
                ? `${bnNum(qN)} ও ${bnNum(qN + 1)}`
                : `${bnNum(qN)}`;
            const pRuns = [
              T(`নিচের উদ্দীপকটি পড় এবং ${hint} নং প্রশ্নের উত্তর দাওঃ\n`, {
                bold: true,
                size: SZ,
              }),
            ];
            if (q.passage)
              pRuns.push(T(q.passage, { size: SZ, italics: true }));
          ...
          if (q.question) {
            const qRuns = [
              T(`${bnNum(qN)}। `, { bold: true, size: SZ }),
              T(q.question, { size: SZ }),
            ];
            ...
            if (qtype === 1 || qtype === 3) {
              ...
              if (q.option_a)
                paras.push(
                  P([T(`   i) ${q.option_a}`, { size: SZ })], {
                    spacing: { after: 20 },
                  }),
                );
              if (q.option_b)
                paras.push(
                  P([T(`   ii) ${q.option_b}`, { size: SZ })], {
                    spacing: { after: 20 },
                  }),
                );
              if (q.option_c)
                paras.push(
                  P([T(`   iii) ${q.option_c}`, { size: SZ })], {
                    spacing: { after: 20 },
                  }),
                );
              paras.push(
                P([T("   নিচের কোনটি সঠিক?", { size: SZ, italics: true })], {
              ...
              if (cr1.length)
                paras.push(
                  P([T("   " + cr1.join("   "), { size: SZ })], {
                    spacing: { after: 30 },
                  }),
                );
              if (cr2.length)
                paras.push(
                  P([T("   " + cr2.join("   "), { size: SZ })], {
                    spacing: { after: 60 },
                  }),
                );
              ...
              if (qtype === 4 && q.question2) {
                ...
                if (q.q2_type === "point") {
                  if (q.q2_option_a)
                    paras.push(
                      P([T(`   i) ${q.q2_option_a}`, { size: SZ })], {
                        spacing: { after: 20 },
                      }),
                    );
                  if (q.q2_option_b)
                    paras.push(
                      P([T(`   ii) ${q.q2_option_b}`, { size: SZ })], {
                        spacing: { after: 20 },
                      }),
                    );
                  if (q.q2_option_c)
                    paras.push(
                      P([T(`   iii) ${q.q2_option_c}`, { size: SZ })], {
                        spacing: { after: 20 },
                      }),
                    );
                  paras.push(
                    P(
                      [T("   নিচের কোনটি সঠিক?", { size: SZ, italics: true })],
                      { spacing: { before: 20, after: 30 } },
                    ),
                  );
                  const q2cb = [
                  ...
                  if (qr1.length)
                    paras.push(
                      P([T("   " + qr1.join("   "), { size: SZ })], {
                        spacing: { after: 30 },
                      }),
                    );
                  if (qr2.length)
                    paras.push(
                      P([T("   " + qr2.join("   "), { size: SZ })], {
                        spacing: { after: 60 },
                      }),
                    );
                  ...
                  answerKey.push({
                    num: qN,
                    label: ansMap2[String(q.q2_answer)] || "?",
                  });
                } else {
                  const qr1 = [],
                    qr2 = [];
                  ...
                  answerKey.push({
                    num: qN,
                    label: String(q.q2_answer || "").toUpperCase() || "?",
                  });
                }
              }
            }
        ...
        if (answerKey.length > 0) {
          const ansChildren = [];
          ansChildren.push(
            P([T("উত্তরমালা", { bold: true, size: 28 })], {
              alignment: AlignmentType.CENTER,
              spacing: { after: 200 },
            }),
          );
          ...
          for (let i = 0; i < answerKey.length; i += CHUNK) {
            const row = answerKey
              ...
              .join("   ");
            ansChildren.push(
              P([T(row, { size: 22 })], { spacing: { after: 80 } }),
            );
          }
      ...
      const safeTitle = (exam || "question-paper").replace(
        /[^a-zA-Z0-9\u0980-\u09FF _-]/g,
        "_",
      );
      res.setHeader(
    ...
    try {
        ...
        if (isMcq) {
          ...
          if (type === 3 || type === 4) {
            if (q.passage || q.passage_image) {
              const hint =
                type === 4
                  ? `${bnIndex1} ও ${toBengaliNumber(qNum + 1)}`
                  : `${bnIndex1}`;
              doc.text(
                `নিচের উদ্দীপকটি পড়ো এবং ${hint} নং প্রশ্নের উত্তর দাও:`,
              );
          ...
          const renderOptions = (a, b, c, d) => {
            const row1 =
              (a ? `ক) ${a}` : "").padEnd(30, " ") + (b ? `    খ) ${b}` : "");
            const row2 =
              (c ? `গ) ${c}` : "").padEnd(30, " ") + (d ? `    ঘ) ${d}` : "");
      ...
      if (!res.headersSent) {
        res
          .status(500)
          .send(
            "Error generating PDF due to complex font parsing. Please try again.",
          );
      }
    }
  }
  ...
  function generateTXT(req, res, data, title, isMcq, showAnswer = false) {
    res.setHeader(
      "Content-disposition",
      `attachment; filename="${title.replace(/\s+/g, "_")}.txt"`,
    );
    ...
    try {
      ...
      data.forEach((q) => {
        ...
        if (isMcq) {
          ...
          if (type === 3 || type === 4) {
            if (q.passage || q.passage_image) {
              ...
              const passageRuns = [
                new TextRun({
                  text: `নিচের উদ্দীপকটি পড়ো এবং ${hint} নং প্রশ্নের উত্তর দাও:`,
                  bold: true,
                  size: 24,
                }),
              ];
          ...
          const appendOptionsDocx = (a, b, c, d) => {
            ...
            if (row1.trim())
              blockChildren.push(
                new Paragraph({
                  children: createTextRuns(row1, { size: 22 }),
                  spacing: { after: 40 },
                }),
              );
            if (row2.trim())
              blockChildren.push(
                new Paragraph({
                  children: createTextRuns(row2, { size: 22 }),
                  spacing: { after: 80 },
                }),
              );
          };
          ...
          if (type === 1 || type === 3) {
            ...
            if (q.option_a)
              blockChildren.push(
                new Paragraph({
                  children: [
                    new TextRun({ text: `i) ${q.option_a}`, size: 22 }),
                  ],
                }),
              );
            if (q.option_b)
              blockChildren.push(
                new Paragraph({
                  children: [
                    new TextRun({ text: `ii) ${q.option_b}`, size: 22 }),
                  ],
                }),
              );
            if (q.option_c)
              blockChildren.push(
                new Paragraph({
                  children: [
                    new TextRun({ text: `iii) ${q.option_c}`, size: 22 }),
                  ],
                }),
              );
            blockChildren.push(
            ...
            if (q.q2_type === "point") {
              if (q.q2_option_a)
                blockChildren.push(
                  new Paragraph({
                    children: [
                      new TextRun({ text: `i) ${q.q2_option_a}`, size: 22 }),
                    ],
                  }),
                );
              if (q.q2_option_b)
                blockChildren.push(
                  new Paragraph({
                    children: [
                      new TextRun({ text: `ii) ${q.q2_option_b}`, size: 22 }),
                    ],
                  }),
                );
              if (q.q2_option_c)
                blockChildren.push(
                  new Paragraph({
                    children: [
                      new TextRun({ text: `iii) ${q.q2_option_c}`, size: 22 }),
                    ],
                  }),
                );
              blockChildren.push(
          ...
          if (q.knowledge) {
            const kRuns = [
              new TextRun({ text: `ক) ${q.knowledge}`, size: 22 }),
            ];
            if (showAnswer && q.answer_k)
              kRuns.push(...formatAnsDocx(q.answer_k));
            blockChildren.push(
              new Paragraph({ children: kRuns, spacing: { after: 80 } }),
            );
          }
          if (q.understanding) {
            const uRuns = [
              new TextRun({ text: `খ) ${q.understanding}`, size: 22 }),
            ];
            if (showAnswer && q.answer_u)
              uRuns.push(...formatAnsDocx(q.answer_u));
            blockChildren.push(
              new Paragraph({ children: uRuns, spacing: { after: 80 } }),
            );
          }
          if (q.application) {
            const aRuns = [
              new TextRun({ text: `গ) ${q.application}`, size: 22 }),
            ];
            if (showAnswer && q.answer_a)
              aRuns.push(...formatAnsDocx(q.answer_a));
            blockChildren.push(
              new Paragraph({ children: aRuns, spacing: { after: 80 } }),
            );
          }
          if (q.higher_thinking) {
            const hRuns = [
              new TextRun({ text: `ঘ) ${q.higher_thinking}`, size: 22 }),
            ];
            if (showAnswer && q.answer_h)
              hRuns.push(...formatAnsDocx(q.answer_h));
            blockChildren.push(
              new Paragraph({ children: hRuns, spacing: { after: 80 } }),
            );
          }
          qNum++;
        }
        ...
        docChildren.push(
          new Paragraph({
            children: [
              new TextRun({ text: "━".repeat(50), color: "E5E7EB", size: 16 }),
            ],
            spacing: { after: 120 },
          }),
        );
  ...
  app.put("/api/students/:roll", (req, res) => {
    ...
    if (index !== -1) {
      ...
      // Sync performance data if it exists
      const perfData = readJson(perfFile);
      const pIndex = perfData.findIndex((p) => String(p.roll) === req.params.roll);
      if (pIndex !== -1) {
        perfData[pIndex].name = data[index].name;
        perfData[pIndex].session = data[index].session;
        perfData[pIndex].class = data[index].class;
        perfData[pIndex].section = data[index].section;
        perfData[pIndex].group = data[index].group;
        perfData[pIndex].gender = data[index].gender;
  ...
  app.post(
    ...
    (req, res) => {
      ...
      let updated = false;
      perfData.forEach((p) => {
        const s = newRows.find((st) => String(st.roll) === String(p.roll));
        if (s) {
          if (
            p.name !== s.name ||
            p.session !== s.session ||
  ...
  app.get("/api/students/demo-xlsx", (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="student-bank-demo.xlsx"',
    );
    res.setHeader(
      "Content-Type",
  ...
  app.post("/api/performance/init", (req, res) => {
    ...
    students.forEach((s) => {
      const exists = perfData.find(
        (p) =>
          String(p.roll) === String(s.roll) && String(p.id) === String(s.id),
      );
      if (!exists) {
        perfData.push({
          id: s.id,
    ...
    const initPayload = students.map((s) => {
      ...
      const attCount = (pData.attendance || []).filter(
        (a) => a.isPresent,
      ).length;
      const attScore = attCount * 5;
  ...
  app.post("/api/performance/attendance", (req, res) => {
    ...
    console.log(
      `[ATTENDANCE GRID SAVE] Date: ${normTargetDate}, records count: ${records ? records.length : 0}`,
    );

    records.forEach((r) => {
      const student = perfData.find(
        (p) =>
          String(p.roll) === String(r.roll) && String(p.id) === String(r.id),
      );
      if (student) {
        if (!student.attendance) student.attendance = [];
        const existingDate = student.attendance.find(
          (a) => normalizeDate(a.date) === normTargetDate,
        );
        if (existingDate) {
  ...
  app.post(
    "/api/performance/attendance/bulk",
    excelUpload.single("file"),
    (req, res) => {
      if (!req.file) return res.status(400).json({ error: "No file uploaded" });
      const newRows = readExcel(req.file.path);
      ...
      newRows.forEach((r) => {
        console.log(
          `[ATTENDANCE BULK ROW] Keys: ${Object.keys(r).join(", ")} | name: ${r.name} | isPresent: ${r.isPresent} | id: ${r.id} | roll: ${r.roll}`,
        );
        ...
        if (student) {
          ...
          const rawPresent =
            r.isPresent !== undefined
              ? r.isPresent
              : r.Present !== undefined
                ? r.Present
                : r.present;
          const isPresent =
            String(rawPresent).trim().toUpperCase() === "TRUE" ||
            String(rawPresent).trim().toUpperCase() === "PRESENT" ||
            String(rawPresent).trim().toUpperCase() === "P" ||
            rawPresent === 1 ||
            rawPresent === true;
          console.log(
            `[ATTENDANCE BULK] Roll: ${student.roll}, Name: ${student.name}, rawDate: ${rawDate}, recordDate: ${recordDate}, rawPresent: ${rawPresent}, isPresent: ${isPresent}`,
          );
          if (!student.attendance) student.attendance = [];
          const existingIdx = student.attendance.findIndex(
            (a) => normalizeDate(a.date) === recordDate,
          );
          if (existingIdx >= 0) {
  ...
  app.get("/api/performance/marks/demo-xlsx", (req, res) => {
    ...
    let rows = students.map((s) => {
      ...
      markCols.forEach((c) => {
        row[c] = marks[c] !== undefined ? marks[c] : 0;
      });
      return row;
    });
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="marks-bulk-template.xlsx"',
    );
    res.setHeader(
      "Content-Type",
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
  ...
  app.get("/api/performance/attendance/demo-xlsx", (req, res) => {
    ...
    let rows = students.map((s) => {
      const perf = perfData.find((p) => String(p.roll) === String(s.roll));
      const attEntry =
        perf && perf.attendance
          ? perf.attendance.find((a) => a.date === targetDate)
          : null;
      return {
        roll: s.roll,
        name: s.name,
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="attendance-bulk-template.xlsx"',
    );
    res.setHeader(
      "Content-Type",
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
